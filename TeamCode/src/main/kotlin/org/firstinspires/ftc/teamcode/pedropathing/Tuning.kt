...
import org.firstinspires.ftc.teamcode.pedropathing.PedroConstants.createFollower
...
internal class LocalizationTest : OpMode() {
    override fun init() {
        Tuning.Companion.follower!!.setStartingPose(Pose(72.0, 72.0))
    }
    ...
    override fun loop() {
        ...
        Tuning.Companion.telemetryM!!.debug(
            "Multiplier: " + (DISTANCE / ((Tuning.Companion.follower!!.getPose()
                .getX() - 72) / Tuning.Companion.follower!!.getPoseTracker().getLocalizer()
...
internal class LateralTuner : OpMode() {
    ...
    override fun loop() {
        ...
        Tuning.Companion.telemetryM!!.debug(
            "Multiplier: " + (DISTANCE / ((Tuning.Companion.follower!!.getPose()
                .getY() - 72) / Tuning.Companion.follower!!.getPoseTracker().getLocalizer()
...
internal class ForwardVelocityTuner : OpMode() {
    ...
    override fun loop() {
        ...
        if (!end) {
            if (abs(Tuning.Companion.follower!!.getPose().getX()) > (DISTANCE + 72)) {
                end = true
                Tuning.Companion.stopRobot()
            } else {
...
internal class LateralVelocityTuner : OpMode() {
     ...
     */
    override fun init_loop() {
        Tuning.Companion.telemetryM!!.debug("The robot will run at 1 power until it reaches " + DISTANCE + " inches to the left.")
        Tuning.Companion.telemetryM!!.debug("Make sure you have enough room, since the robot has inertia after cutting power.")
    ...
    override fun loop() {
        ...
        if (!end) {
            if (abs(Tuning.Companion.follower!!.getPose().getY()) > (DISTANCE + 72)) {
                end = true
                Tuning.Companion.stopRobot()
            } else {
...
internal class LateralZeroPowerAccelerationTuner : OpMode() {
    ...
    override fun init_loop() {
        Tuning.Companion.telemetryM!!.debug("The robot will run to the left until it reaches " + VELOCITY + " inches per second.")
        Tuning.Companion.telemetryM!!.debug("Then, it will cut power from the drivetrain and roll to a stop.")
        Tuning.Companion.telemetryM!!.debug("Make sure you have enough room.")
...
internal object Drawing {
     ...
     */
    fun drawPoseHistory(poseTracker: PoseHistory, color: String?) {
        if (packet == null) packet = TelemetryPacket()
        packet!!.fieldOverlay().setStroke(color)
        packet!!.fieldOverlay()
    ...
    fun drawRobotOnCanvas(c: Canvas, t: Pose?) {
        ...
        val v = t.getHeadingAsUnitVector()
        v.setMagnitude(v.getMagnitude() * ROBOT_RADIUS)
        val x1 = t.getX() + v.getXComponent() / 2
        val y1 = t.getY() + v.getYComponent() / 2
        val x2 = t.getX() + v.getXComponent()
        val y2 = t.getY() + v.getYComponent()
